...
class ImprovedObjectTracker:
    ...
    def setup_ui(self):
        ...
        self.load_btn = tk.Button(control_frame, text="Load Template", command=self.load_template)
        self.load_btn.pack(side=tk.LEFT, padx=5)
        
        self.start_btn = tk.Button(control_frame, text="Start Camera", command=self.start_camera)
        self.start_btn.pack(side=tk.LEFT, padx=5)
        
        self.stop_btn = tk.Button(control_frame, text="Stop", command=self.stop_camera, state=tk.DISABLED)
        self.stop_btn.pack(side=tk.LEFT, padx=5)
        
        # Выбор метода обнаружения
        tk.Label(control_frame, text="Detection:").pack(side=tk.LEFT, padx=10)
        ...
        self.status_label = tk.Label(control_frame, text="Status: Load template and start camera")
        ...
        # Левая панель - видео
        left_frame = tk.LabelFrame(content_frame, text="Webcam Feed", padx=10, pady=10)
        ...
        self.canvas_text = self.video_canvas.create_text(320, 240, 
                                                        text="Camera will start here\n\nClick 'Start Camera'", 
                                                        fill="white", font=("Arial", 14), justify=tk.CENTER)
        
        # Правая панель - шаблон и информация
        right_frame = tk.LabelFrame(content_frame, text="Template & Info", padx=10, pady=10)
        right_frame.pack(side=tk.RIGHT, fill=tk.BOTH, padx=5)
        
        self.template_label = tk.Label(right_frame, text="No template loaded\n\nClick 'Load Template'", 
        ...
        self.info_text.insert(tk.END, "Detection Info:\n\n")
...
1. Load template image (object to track)
2. Select detection method:
   - Feature Matching: Best for objects with texture
   - Template Matching: Simple but sensitive to changes
   - Color Detection: For objects with distinct color
3. Start camera and draw rectangle if auto-detection fails
4. Object will be tracked with green box"""
        
        tk.Label(bottom_frame, text=instructions, justify=tk.LEFT, font=("Arial", 10)).pack()
    ...
    def on_method_change(self, event):
        ...
        self.status_label.config(text=f"Method changed to: {self.detection_method}")
    ...
    def load_template(self):
        ...
        if file_path:
            try:
                ...
                self.update_info(f"Template loaded:\nSize: {self.template.shape[1]}x{self.template.shape[0]}\nFeatures: {len(self.kp_template) if self.kp_template else 0}")
                
                self.status_label.config(text="Template loaded - Click 'Start Camera'")
    ...
    def update_info(self, text):
        ...
        self.info_text.insert(tk.END, f"Detection Info:\n\n{text}")
        self.info_text.config(state=tk.DISABLED)
    ...
    def start_camera(self):
        ...
        if self.template is not None:
            self.status_label.config(text=f"Camera started - Using {self.detection_method}")
        else:
            self.status_label.config(text="Camera started - Load template for tracking")
        ...
        if self.drawing and self.current_frame is not None:
            ...
            if abs(x2 - x1) > 20 and abs(y2 - y1) > 20:
                ...
                if canvas_width > 1 and canvas_height > 1:
                    ...
                    if success:
                        self.status_label.config(text="Tracking: Manual selection started")
                        self.update_info("Manual selection started")
    ...
    def update_frame(self):
        ...
        if self.tracker is None and self.template is not None:
            ...
            if detected:
                self.status_label.config(text=f"Tracking: {self.detection_method} detected (conf: {confidence:.2f})")
                self.update_info(f"Auto-detected with {self.detection_method}\nConfidence: {confidence:.2f}")
                ...
                info_text = f"{self.detection_method} failed\nConfidence: {confidence:.2f}\nDraw rectangle manually"
        ...
        if self.tracker is not None:
            ...
            if success:
                ...
                self.status_label.config(text="Tracking: Object tracked")
            else:
                self.status_label.config(text="Tracking lost - draw rectangle again")
                self.tracker = None
